...
class ReparamModule(nn.Module):
    ...
    def _embed_with_param_and_buffers(self, flat_param, buffers, *inputs, **kwinputs):
        with self.unflattened_param(flat_param):
            with self.replaced_buffers(buffers):
                return self.module.embed(*inputs, **kwinputs)

    def _embed_with_param(self, flat_param, *inputs, **kwinputs):
        with self.unflattened_param(flat_param):
            return self.module.embed(*inputs, **kwinputs)

    def embed(self, *inputs, flat_param=None, buffers=None, **kwinputs):
        flat_param = torch.squeeze(flat_param)
        if flat_param is None:
            flat_param = self.flat_param
        if buffers is None:
            return self._embed_with_param(flat_param, *inputs, **kwinputs)
        else:
            return self._embed_with_param_and_buffers(flat_param, tuple(buffers), *inputs, **kwinputs)